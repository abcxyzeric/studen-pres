        ...
        if (command === pluginName) {
            switch (args[0]) {
				case "setChoices":  // 選択肢の定義
                    //---Added code---
                    let st = String(args[1].replace(/_/g, " "));
                    //var choices = args[1].split(',');//original code
                    var choices = st.split(',');
                    //----------------
                    choiceLists = [];
                    if (choices) {
                ...
                case "setMessageText":  // 質問メッセージの定義
                    ...
                    if (messageText) {
                        //---Added code---
                        //messageTextLists.push(String(args[1] || ""));//original code
                        messageText = messageText.replace(/_/g, " ");
                        messageTextLists.push(messageText || "");
                        //----------------
                    }
                    return;
