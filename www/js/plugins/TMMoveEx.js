    ...
    if (!this.isMovementSucceeded()) {
      ...
      if (!$gameMap.isPassable(this.x, this.y, d) || !$gameMap.isPassable(x2, y2, d2)) {
        ...
        if (this._knockWallCount + knockWallInterval <= Graphics.frameCount ||
            ...
            //unit_see追記           
            if ($gamePlayer.x != playerMoveX || $gamePlayer.y != playerMoveY || Game_Player.prototype.getInputDirection() != playerMoveD) {
                AudioManager.playSe(knockWallSe);
                playerMoveX = $gamePlayer.x
                playerMoveY = $gamePlayer.y
                playerMoveD = Game_Player.prototype.getInputDirection()
            }
