...
/*:ja
 * @plugindesc (v1.1) 要MOG_SceneMenu。アイテムシーンを装飾します。
 * @author Moghunter
 *
 * @param -> Window <<<<<<<<<<<<<<<<<<<<<<<
 * @text -> ウィンドウ <<<<<<<<<<<<<<<<<<<<<<<
 ...
 * ===========================================================================
 * アイテムシーンを装飾します。
 * このプラグインは動作するためにMOG_SceneMenuを必要とします。
 *
 * ===========================================================================
 * 必要画像ファイル
 * ===========================================================================
	...
	for (var i = 0; i < this._buttons.length; i++) {
		if (this._categoryWindow._index === i && !this._actorWindow.active) {
			if (this._buttonsAni[i] === 0) {
				...
				if (this._buttons[i].scale.x >= 1.15) {
					this._buttons[i].scale.x = 1.15;
...
Scene_Item.prototype.checkTouchOnSprites = function () {
	if (this._itemWindow.active) {
		...
		if (!touch) {
			for (var i = 0; i < this._buttons.length; i++) {
				if (this.isOnSprite(this._buttons[i])) {
					this.setTouchType(i)
...
Scene_Item.prototype.refreshStatusActor = function () {
	this._actorStatusIndex = this._actorWindow._index;
	this._actorStatus = $gameParty.members()[this._actorWindow._index];
	if (this._actorStatus && !this._actorDataWindow) {
...
PartyWindowData.prototype.createPar = function () {
	this._par = new Sprite(new Bitmap(320, 190));
...
PartyWindowData.prototype.refreshPar = function () {
	...
	this._par.bitmap.drawText(this._actor.hp, Moghunter.scItem_PartyHPNumberX, Moghunter.scItem_PartyHPNumberY, 170, 80, 32, "right");
	this._par.bitmap.drawText(this._actor.mp, Moghunter.scItem_PartyMPNumberX, Moghunter.scItem_PartyMPNumberY, 70, 200, 32, "right");
	this._par.bitmap.drawText(this._actor.name(), Moghunter.scItem_PartyNameX, Moghunter.scItem_PartyNameY, 75, 260, 32, "left");
};
...
ActorDataWindow.prototype.refreshParameters = function () {
	this._par.bitmap.clear();
	this._par.bitmap.drawText(this._actor.hp, -20, 76, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.mhp, 60, 76, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.mp, -20, 110, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.mmp, 60, 110, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.atk, -10, 182, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.def, 160, 182, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.mat, -10, 220, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.mdf, 160, 220, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.agi, -10, 255, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.luk, 160, 255, 160, 32, "right");
	this._par.bitmap.drawText(this._actor.name(), 70, 0, 160, 32, "left");
};
