 ...
 * @option 1:固定（常に表示）
 * @value 1
 * @option 2:装備（装備品に含まれるステータス）
 * @value 2
 * @option 3:変動（変更があるステータス）
 * @value 3
 * @option 4:装備or変動
 * @value 4
...

{
    ...
    Params.statusWindowRow = Number(parameters['Status Window Row'] || 7);
    Params.gaugeHeight = Number(parameters['Gauge Height'] || 8);
    Params.GaugeColor = 'rgb(%1)'.format(parameters['Gauge Color'] || '224,255,255');
    Params.GaugeType = Number(parameters['Gauge Type'] || 0);
    Params.GaugeSlope = Number(parameters['Gauge Slope'] || 0.5);

    // Gauge Max
    let param = JSON.parse(parameters['param Gauge Max'] || "{}");
    Params.paramGaugeMax = [];
    Params.paramGaugeMax[0] = Number(param.mhp);
    Params.paramGaugeMax[1] = Number(param.mmp);
    Params.paramGaugeMax[2] = Number(param.atk);
    Params.paramGaugeMax[3] = Number(param.def);
    Params.paramGaugeMax[4] = Number(param.mat);
    Params.paramGaugeMax[5] = Number(param.mdf);
    Params.paramGaugeMax[6] = Number(param.agi);
    Params.paramGaugeMax[7] = Number(param.luk);
    Params.MaxSquare = !!eval(parameters['Max Square']);
    Params.xparamGaugeMax = Number(parameters['xparam Gauge Max'] || 2.0);
    Params.sparamGaugeMax = Number(parameters['sparam Gauge Max'] || 2.0);
    Params.rateGaugeMax = Number(parameters['rate Gauge Max'] || 2.0);

    // Status
    Params.defaultStatus = convertParam(parameters['Default Status']);
    Params.fixingStatus = convertParam(parameters['Fixing Status']);
    Params.itemStatus = convertParam(parameters['Item Status']);
    Params.flowStatus = convertParam(parameters['Flow Status']);
    ...
    Params.OriginalTraitType = Number(parameters['Original Trait Type'] || 4);
    
    // Terms
    Params.terms = {};
    param = JSON.parse(parameters['xparams'] || "{}");
    Params.terms[8]  = param.hit;
    Params.terms[9]  = param.eva;
    Params.terms[10] = param.cri;
    Params.terms[11] = param.cev;
    Params.terms[12] = param.mev;
    Params.terms[13] = param.mrf;
    Params.terms[14] = param.cnt;
    Params.terms[15] = param.hrg;
    Params.terms[16] = param.mrg;
    Params.terms[17] = param.trg;
    param = JSON.parse(parameters['sparams'] || "{}");
    Params.terms[18] = param.tgr;
    Params.terms[19] = param.grd;
    Params.terms[20] = param.rec || "回復効果率";
    Params.terms[21] = param.pha;
    Params.terms[22] = param.mcr;
    Params.terms[23] = param.tcr;
    Params.terms[24] = param.pdr;
    Params.terms[25] = param.mdr;
    Params.terms[26] = param.fdr;
    Params.terms[27] = param.exr;
    Params.terms.elementRate = parameters['Element Rate'] || '';
    Params.terms.debuffRate = parameters['Debuff Rate'] || '';
    Params.terms.stateRate = parameters['State Rate'] || '';
    Params.terms.stateResist = parameters['State Resist'] || '';
    
    Params.downStrongParams = [22,24,25,26];
    
    // Command
    Params.EquipMetadata = JSON.parse(parameters['Equip Metadata'] || "{}");
...
Game_BattlerBase.prototype.allOriginalTraits_mpp = function() {
    var result = [];
    var name = Params.EquipMetadata.mppEqSt || "mppEqSt";
...
Window_EquipStatus.prototype.createElementRateList = function(maxRow) {
    ...
    if (Params.terms.elementRate && type > 0) {
        var code = Game_BattlerBase.TRAIT_ELEMENT_RATE;
        var elements = Params.ElementRate.DrawIds;
...
Window_EquipStatus.prototype.createDebuffRateList = function(maxRow) {
    ...
    if (Params.terms.debuffRate && type > 0) {
        var code = Game_BattlerBase.TRAIT_DEBUFF_RATE;
        var debuffs = Params.DebuffRate.DrawParams;
...
Window_EquipStatus.prototype.createStateRateList = function(maxRow) {
    ...
    if (Params.terms.stateRate && type > 0) {
        var code = Game_BattlerBase.TRAIT_STATE_RATE;
        var states = Params.StateRate.DrawIds;
...
Window_EquipStatus.prototype.createStateResistList = function(maxRow) {
    ...
    if (Params.terms.stateResist && type > 0) {
        var code = Game_BattlerBase.TRAIT_STATE_RESIST;
        var states = Params.StateResist.DrawIds;
...
Window_EquipStatus.prototype.drawFlatGauge = function(x, y, curWidth, newWidth, color) {
    ...
    context.save();
    context.setTransform(1, 0, Params.GaugeSlope, 1, x, gy);
    ...
    this.drawFlatLine(0, 0, curW, Params.GaugeColor);
...
Window_EquipStatus.prototype.drawFlatLine = function(x, y, width, color) {
    ...
    var context = this.contents.context;
    //context.save();
    //context.setTransform(1, 0, Params.GaugeSlope, 1, x, y);
...
Window_EquipStatus.prototype.drawArcGauge = function(x, y, curWidth, newWidth, color) {
    ...
    this.drawArcLine(0, 0, curW, Params.GaugeColor);
...
Window_EquipStatus.prototype.draw2LineGauge = function(x, y, curWidth, newWidth, color) {
    ...
    if (color === Params.GaugeColor) {
        this.drawFlatLine(0, 0, curWidth, Params.GaugeColor);
    } else {
        this.drawFlatLine(0, -gh, curWidth, Params.GaugeColor);
...
Window_EquipStatus.prototype.drawParam = function(x, y, paramId) {
    ...
    if (Params.MaxSquare && paramId < 8) {
        curValue = Math.sqrt(curValue);
        newValue = Math.sqrt(newValue);
        max = Math.sqrt(max);
    }
    var reverse = Params.downStrongParams.contains(paramId);
...
Window_EquipStatus.prototype.drawParamName = function(x, y, paramId) {
    if (paramId < 8) {
        ...
        this.drawText(Params.terms[paramId], x, y, 120);
    }
};
...
Window_EquipStatus.prototype.drawResist = function(x, y, id) {
    var name = Params.terms.stateResist.format($dataStates[id].name);
