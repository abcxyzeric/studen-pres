...
/*:
 * @plugindesc (v1.1) Modifica a cena de habilidades.
 * @author Moghunter
 *
 * @param List Width
 ...
 * HISTÓRICO
 * ===========================================================================
 * (v1.1) - Correção do plugin parameter relativo ao SW Par Y-Axis.
 */
/*:ja
...
Moghunter.scSkill_AS_ParY = Number(Moghunter.parameters['SW Par Y-Axis'] || 0);
...
Scene_Skill.prototype.updateActorStatusWindow = function () {
	if (this._ActorStatusWindow.y < this._ActorOrg[1]) {
		this._ActorStatusWindow.y += 5
...
Scene_Skill.prototype.updateCommands = function () {
	...
	if (this._itemWindow.active) {
		if (Input.isTriggered('right')) { this._wani = [0, 0, null]; SoundManager.playCursor() };
		if (Input.isTriggered('left')) { this._wani = [0, 0, null]; SoundManager.playCursor() };
	} else {
...
ActorStatusSkill.prototype.refreshPar = function () {
	...
	this._par.bitmap.drawText(this._actor.hp, 100, 12, 80, 32, "right");
	this._par.bitmap.drawText(this._actor.mp, 135, 78, 80, 32, "right");
	this._par.bitmap.drawText(this._actor.name(), 0, 140, 120, 32, "left");
};
